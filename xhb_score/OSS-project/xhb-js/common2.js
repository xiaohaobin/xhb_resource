 ...
 * ajaxFn         普通ajax请求
 * ajaxFn1  普通ajax请求,(加上同步异步，显示加载过程）
...
var oComFn = {
	 ...
	 * */
	comLaydate: function(selector, sType, fn, isR, color) {
		if (sType == "date") {
	...
	layuiTableConfing: {
		...
		text: {
			none: xhb_oss_spring.lang.no_data, //默认：无数据。注：该属性为 layui 2.2.5 开始新增
			// none:nodataHtml()
		},
		cols: [],
	},
	...
	layerSucc: function(fn) {
		layer.msg(xhb_oss_spring.lang.operat_susccess, {
			time: 1500
		}, function() { //"操作成功"
			fn && fn();
		});
     ...
     * @param {String} method 请求类型
     * @param {Function} successFn 请求成功的回调函数
     * @param {Boolean} async 同步、异步
     * */
    ajaxFn1:function(murl, mdata, method, successFn,async){
        var _this = this;
        $.ajax({
            type: method,
            url: murl,
	...
	sysAlert: function(msg, fn) {
		var o = layer.alert(
			msg, {
				title: xhb_oss_spring.lang.hr_hint,//"提示",
				btn: [xhb_oss_spring.lang.determine]//"确定"
			},
			function() {
				layer.close(o);
				fn && fn();
	...
	delConfirmFn: function(fn1, fn2) {
		var index = layer.confirm(
			xhb_oss_spring.lang.xhb_Confirm_delete, {
				title: xhb_oss_spring.lang.hr_hint,
				btn: [xhb_oss_spring.lang.determine, xhb_oss_spring.lang.cancel]
			},
			function(a, b) {
				fn1(a, b);
			},
	...
	noMoreClick: function(obj) {
		...
		if (clickTime != 'undefined' && (nowTime - clickTime < 5000)) {
			$.isMoblie(
				function() {
					mui.alert(xhb_oss_spring.lang.ksh_Dont_do_it_too_often);
				},
				function() {
					layer.alert(xhb_oss_spring.lang.ksh_Dont_do_it_too_often);
				}
			);
	...
	sys_copy_fn: function(str, fn) {
		...
		oInput.remove();
		layer.msg(xhb_oss_spring.lang.copy_right);//复制成功
		fn && fn(str);
	},
	/**
	 * 自定义Validform插件提示语
	...
	input_ShowTitle_vaild: function() {
		$("input").each(function() {
			...
			if ($(this).attr("nullmsg")) { //设置公共正则：不能为空
				$(this).attr("nullmsg", xhb_oss_spring.lang.not_empty);//"不能为空"
			}

			if ($(this).attr("datatype") && $(this).attr("datatype") == "n") { //设置公共正则：请输入正整数
				$(this).attr("errormsg", xhb_oss_spring.lang.xhb_Only_positive_integers_can_be_entered);//"只能输入正整数"
			}

			if ($(this).attr("datatype") && $(this).attr("datatype") == "*2-18" && $(this).attr("recheck") == undefined) { //设置公共正则：请输入2-18个字符
				$(this).attr("errormsg", xhb_oss_spring.lang.xhb_Please_enter_2_18_characters);//"请输入2-18个字符"
			}
			if ($(this).attr("datatype") && $(this).attr("datatype") == "*2-30") { //设置公共正则：请输入2-30个字符
				$(this).attr("errormsg", xhb_oss_spring.lang.xhb_Please_enter_2_30_characters);//"请输入2-30个字符"
			}
			if ($(this).attr("datatype") && $(this).attr("datatype") == "*2-100") { //设置公共正则：请输入2-100个字符
				$(this).attr("errormsg", xhb_oss_spring.lang.xhb_Please_enter_2_100_characters);//"请输入2-100个字符"
			}
			if ($(this).attr("datatype") && $(this).attr("datatype") == "e") { //设置公共正则：请输入正确的邮箱格式字符
				$(this).attr("errormsg", xhb_oss_spring.lang.xhb_Please_correct_email_format_characters);//"请输入正确的邮箱格式字符"
			}
			if ($(this).attr("datatype") && $(this).attr("datatype") == "url") { //设置公共正则：请输入正确的域名
				$(this).attr("errormsg", xhb_oss_spring.lang.xhb_Please_enter_the_correct_domain_name);//"请输入正确的域名"
			}
			if ($(this).attr("datatype") && $(this).attr("datatype") == "m") { //设置公共正则：请输入正确的手机号
				$(this).attr("errormsg", xhb_oss_spring.lang.please_correct_phone_number);//"请输入正确的手机号"
			}
		});
	},
	...
	chart_option_1:{//曲线图
		...
		series: [],
		lang: {
			noData: xhb_oss_spring.lang.no_data,
	...
	echart_nodata_todo2: function(data, n, chartId) {
		...
		echart_ondata_box.append($('<div>'+ xhb_oss_spring.lang.no_data +'</div>'));
		if (isNodata) {
	...
	minNumNotice: function(fn) {
		layer.msg(xhb_oss_spring.lang.xhb_Check_at_least_one); //起码勾选一个
		fn && fn();
	},
	//判断是否当前浏览器为IE
	isIE: function() {
	...
	//number输入框递增递减按钮事件初始化
	numInputEventInit:function(){
		$(".sys_numInput_box .sys_numInput_box_left").on("click",function(){
			var numInput = $(this).siblings(".sys_input");
			numInput.val( numInput.val() * 1 - 1 );
		});
		$(".sys_numInput_box .sys_numInput_box_right").on("click",function(){
			var numInput = $(this).siblings(".sys_input");
			numInput.val( numInput.val() * 1 + 1 );
		});
	},
	//獲取國家區域公共數據字典
	getAreaCountryName:function(fn){
		
		$.ajax({
			url:xhb_oss_spring.com_root + '/common/getAreaCountryName',
			type:"get",
			data:{},
			success:function(data){
				
				var cData = (typeof data == "object" ? data : JSON.parse(data));
				var cclist = [];
				for(var i=0;i<cData.length;i++){
					cclist.push({
						...
						txt:cData[i].data[1],
						curr:i === 0 ? true : false,
						children:toChildrenData(cData[i])
					});
				}
				
				function toChildrenData(item){
					var list = [];
					for(var i=0;i<item.subs.length;i++){
						list.push({
							pid:item.data[0],
							id:item.subs[i].data[0],
							txt:item.subs[i].data[1]
						});
					}
					return list;
				}
				xhb_oss_spring.areaAndCountryList = cclist;
				fn && fn(cclist);
			}
		});
	},
	...
	//oss公共單個文件上傳函數
	oss_file_upload:function(formData,fn){
		this.ajaxFn_file(
			xhb_oss_spring.com_root + '/common/upload',
			formData,
			"post",
			function(data){
				console.log(data,"optionRule");
				if(data.code == "0"){
					fn && fn(data);
				}
				else{
					layer.msg(xhb_oss_spring.lang.xhb_Image_upload_failed);//"图片上传失败，请稍后再上传"
				}
	...
	loadContent:function(url){
		//loadContent(url,null,null,true);	
		loadContent(url);	
	},
	//oss公共图片cdn域名和文件名
	...
	AmazonSiteList:[//亚马逊站点列表
		...
		{val:"Amazon.com.mx",txt:"Amazon.com.mx",country:"墨西哥"},
		{val:"Amazon.co.uk",txt:"Amazon.co.uk",country:"英国"},
		{val:"Amazon.de",txt:"Amazon.de",country:"德国"},
		{val:"Amazon.fr",txt:"Amazon.fr",country:"法国"},
		{val:"Amazon.es",txt:"Amazon.es",country:"西班牙"},
		{val:"Amazon.it",txt:"Amazon.it",country:"意大利"},
		{val:"Amazon.ni",txt:"Amazon.ni",country:"荷兰"},
		{val:"Amazon.se",txt:"Amazon.se",country:"瑞典"},
		{val:"Amazon.pt",txt:"Amazon.pt",country:"波兰"},
		{val:"Amazon.co.jp",txt:"Amazon.co.jp",country:"日本"},
		{val:"Amazon.com.au",txt:"Amazon.com.au",country:"澳大利亚"},
		{val:"amazon.sg",txt:"amazon.sg",country:"新加坡"},
		{val:"amazon.ae",txt:"amazon.ae",country:"阿联酋"},
		{val:"amazon.in",txt:"amazon.in",country:"印度"},
	],
	//随机生成任意长度的数组，元素皆为数字
	randomYData:function(n){
		var list = [];
		for(var i=0;i<n;i++){
	...
	//返回上传图片尺寸验证字段；limitOpts.width;limitOpts.height  定义的图片的宽高尺寸像素
	getLimitImgTxt:function(limitOpts){
		 return "上传的图片像素最小必须是：" + limitOpts.width + "*" + limitOpts.height + ";" + "上传的图片像素宽高比例必须是：" + limitOpts.width + ":" + limitOpts.height;
	},
	/**
	limitOpts.width;limitOpts.height  定义的图片的宽高尺寸像素
	opts.width;opts.height图片的宽高尺寸像素
	*/
	vaildUploadImgLimit:function(limitOpts,opts){
		if (limitOpts.width * 1 > opts.width || limitOpts.height * 1 > opts.height) {												
			return false;
		}
		if( (opts.width / limitOpts.width).toFixed(1)*1 !== (opts.height / limitOpts.height).toFixed(1)*1 ){
			return false;
		}
	...
	//验证两个数字区间关系是否正确
	isTrueByNumberRange:function(min,max){
		if(min === '' && max === ''){
			return true;
		}else{
	...
	/**
	 * 根据国家列表id，区域id，返回对应字段列表
	 * @param {Object} oCountryList 对象数据，key为区域的id，值为国家id列表,如{"1":['2']}
	 * @param {Array} worldData 世界区域国家数据：[id:'1',txt:'亚洲',children:[{pid: '1', id: '1', txt: '中国'}...]]
	*/
	getCountryTxtList(oCountryList,worldData){
		let oCountryList2 = JSON.parse( JSON.stringify(oCountryList) );
		worldData.forEach((item,index)=>{
			for(let key in oCountryList2){
				if(key == item.id){
					oCountryList2[key] = getTxt(item.children, oCountryList2[key]);
				}
			}
		});
		return oCountryList2;

		function getTxt(children,idList){
			let arr = [];
			for (let index = 0; index < children.length; index++) {
				const element = children[index];
				if(typeof idList == "object"){
					if(idList.includes(element.id)) arr.push( element.txt );
				}
				else{
					if(element.id === idList) arr.push( element.txt );
				}
			}
			return arr;
		}
	...
	*/
	...
	//根据区域id获取对应字段
	getTxtByAreaId(id, worldData){
		let arr = [];
		let txt = '';
		worldData.forEach((item,index)=>{
			if(typeof id == "object"){
				if( id.includes(item.id) ) arr.push(item.txt);
			}else{
				if(item.id === id) txt = item.txt;
			}
		});

		return (arr.length ? arr.join() : txt);

	},
};
//獲取國家區域公共數據字典
	
oComFn.getAreaCountryName();